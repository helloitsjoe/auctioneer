...
test.each([[1, 0], [null, 0], ['Joe', 0]])('getMinBidValue %% throws', () => {
  expect(utils.getMinBidValue).toThrow('Bids must be an array');
});

describe('getUser', () => {
  it('gets user from session storage', () => {
    expect(utils.getUser()).toBe('');
    window.sessionStorage.setItem('userName', 'Joe');
    expect(utils.getUser()).toBe('Joe');
