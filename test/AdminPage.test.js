...
describe('AdminPage', function() {
  ...
  it('deleting last item replaces item with blank item', function() {
    ...
    wrapper = mount(
      <MemoryRouter initialEntries={['/admin']}>
        <AdminPage initialItems={[firstItem]} deleteRequest={deleteRequest} />
      </MemoryRouter>
    );
  ...
  `(
    ...
    ({ titleValue, descriptionValue, modalLength, callCount }) => {
      ...
      wrapper.find('#title').prop('onChange')({
        target: { value: titleValue },
      });
      wrapper.find('#description').prop('onChange')({
